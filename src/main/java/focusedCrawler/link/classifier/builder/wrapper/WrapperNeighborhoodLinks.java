...
/**
 * <p>
 * Description: This class from a predefined set of words extracts for each link
 * the frequency of these words given a page. These words are the features used
...
public class WrapperNeighborhoodLinks {
     ...
     * 
     * @param page
     *            Page page containing the links to be obtained
     * @param features
     *            String[] pre-defined words
    ...
    public HashMap extractLinks(LinkNeighborhood[] linkNeighboors, String[] features)
            throws MalformedURLException {
        HashMap linkFields = extractLinks(linkNeighboors);
        return mapFeatures(linkFields, features);
    }

    public HashMap extractLinks(LinkNeighborhood linkNeighboor, String[] features)
            throws MalformedURLException {
    ...
    public HashMap extractLinksFull(LinkNeighborhood linkNeighboor, String[] features)
            throws MalformedURLException {
    ...
    private HashMap mapFeatures(HashMap linkFields, String[] features) {
        ...
        while (wordsFields.hasNext()) {
            ...
            for (int j = 0; j < words.length; j++) {
                ...
                if (wordField.getField() == WordField.URLFIELD
                        || wordField.getField() == WordField.SRC) {
    ...
    private HashMap extractLinksFull(LinkNeighborhood ln) throws MalformedURLException {
        ...
        if (ln.getImgSrc() != null) {
            // PaginaURL pageParser = new PaginaURL(new URL("http://"), 0, 0,
            // ln.getImgSrc().length(), ln.getImgSrc(), stoplist);
            PaginaURL pageParser = new PaginaURL(new Page(new URL("http://"), ln.getImgSrc()),
                    stoplist);
    ...
    private HashMap extractLinks(LinkNeighborhood[] linkNeighboors) throws MalformedURLException {
        HashMap result = new HashMap();
    ...
    private HashMap extractLinks(PaginaURL pageParser) throws MalformedURLException {
     ...
     *
     * @param page
     *            Page - page to be scanned
     * @param link
     *            String - link to be found
     * @throws MalformedURLException
     * @return String[] - bag of words close to given hyperlink
     */
    public WordField[] getNeighboorhood(Page page, String link) throws MalformedURLException {
        String pageStr = page.getContent();
        // PaginaURL pageParser = new PaginaURL(page.getURL(), 0, 0,
        // pageStr.length(), pageStr,
        // stoplist);
        PaginaURL pageParser = new PaginaURL(new Page(page.getURL(), pageStr), stoplist);
...
//        PaginaURL pageParser = new PaginaURL(url, 0, 0, url.getFile().length(), url.getFile(),
//                stoplist);
        PaginaURL pageParser = new PaginaURL( new Page(url,url.getFile()),
                stoplist);
        String[] terms = pageParser.palavras();
        for (int i = 0; i < terms.length; i++) {
    ...
    public static void main(String[] args) throws MalformedURLException {
        ...
        try {
            ...
            fieldWords[WordField.URLFIELD] = new String[] { "search", "book", "lib", "shop1",
                    "archiv1" };
            fieldWords[WordField.ANCHOR] = new String[] { "book", "search", "advanc", "librari",
                    "press", "resourc1" };
            fieldWords[WordField.AROUND] = new String[] { "book", "search", "titl", "advanc",
                    "shop1", "resourc1" };
         ...
         * 
         * st = new
         * util.string.StopListArquivo(config.getParam("STOPLIST_FILES"));
         * 
         * }
         * 
         * catch (IOException ex) {
         ...
         * 
         * hwc.submission.Submitter submitter = new
         * hwc.submission.Submitter(downloader);
         * 
         * 
         * 
